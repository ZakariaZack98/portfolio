...
const Footer = () => {
  ...
  return (
    <div className="w-full bg-black">
      <div className="footer bgLayer bg-cover bg-center mx-auto border border-[rgba(255,255,255,0.38)] rounded-2xl pb-10 w-19/20 md:container overflow-hidden">
        <div className="grad-overlay border-b border-[rgba(255,255,255,0.38)]">
          <div className="top flex flex-col lg:items-start items-center gap-y-5 border-b border-[rgba(255,255,255,0.38)] p-10">
            <h1 className="text-4xl md:text-5xl font-bold lg:max-w-6/10 max-w-full text-center lg:text-start leading-12 lg:leading-14">
              Like what you see? Reach out <u className="text-blue-500 cursor-pointer">via email</u> to collaborate{" "}
            </h1>
            <Link href={"/contact"}>
                ...
                <div key={group.name} className="flex flex-col items-end gap-y-2 text-sm md:text-base">
                  ...
                  {group.links?.map((link) => (
                    <Link
                      key={link.label}
                      href={link.path}
                      target={link.path.startsWith("https://") ? "_blank" : "_self"}
                      className="opacity-70 text-sm md:text-base text-end hover:text-blue-500">
                      {link.label}
                    </Link>
                  ))}
                </div>
